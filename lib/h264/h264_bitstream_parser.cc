...
H264BitstreamParser::Result H264BitstreamParser::ParseNonParameterSetNalu(
  ...
  if (nalu_type == 20 || nalu_type == 21) {
    std::cout << "Unsupported nal unit type." <<std::endl;
  ...
  if ((pps_->weighted_pred_flag && (slice_type == H264::SliceType::kP ||
    ...
    std::cout << "Streams with pred_weight_table unsupported." << std::endl;
    return kUnsupportedStream;
  }
  ...
  if (abs(last_slice_qp_delta) > kMaxAbsQpDeltaValue) {
    ...
    std::cout << "Parsed QP value out of range." <<std::endl;
    return kInvalidStream;
  }
...
void H264BitstreamParser::ParseSlice(const uint8_t* slice, size_t length) {
  ...
  switch (nalu_type) {
    case H264::NaluType::kSps: {
      ...
      if (!sps_)
        std::cout << "Unable to parse SPS from H264 bitstream." << std::endl;
      break;
    }
    case H264::NaluType::kPps: {
      pps_ = PpsParser::ParsePps(slice + H264::kNaluTypeSize,
                                 length - H264::kNaluTypeSize);
      if (!pps_)
        std::cout << "Unable to parse PPS from H264 bitstream." << std::endl;
      break;
    }
    ...
    default:
      ...
      if (res != kOk)
        std::cout << "Failed to parse bitstream. Error: " << res << std::endl;
      break;
  }
}
...
bool H264BitstreamParser::GetLastSliceQp(int* qp) const {
  ...
  if (parsed_qp < kMinQpValue || parsed_qp > kMaxQpValue) {
    std::cout << "Parsed invalid QP from bitstream." << std::endl;
    return false;
  }
  *qp = parsed_qp;
  return true;
