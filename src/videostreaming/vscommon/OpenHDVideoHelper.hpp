...
static VideoCodec intToVideoCodec(int videoCodec){
    ...
    std::cout << "VideoCodec::intToVideoCodec::somethingWrong,using H264 as default" <<std::endl;
...
static VideoStreamConfigXX read_from_settingsXX(bool is_primary){
    Settings settings;
    OpenHDVideoHelper::VideoStreamConfigXX _videoStreamConfig;
    if(is_primary){
        ...
        _videoStreamConfig.udp_rtp_input_ip_address=settings.value("qopenhd_primary_video_rtp_input_ip",kDefault_udp_rtp_input_ip_address).toString();
        const int tmp_video_codec = settings.value("qopenhd_primary_video_codec", 0).toInt();
        _videoStreamConfig.video_codec=OpenHDVideoHelper::intToVideoCodec(tmp_video_codec);
        ...
        _videoStreamConfig.udp_rtp_input_ip_address=settings.value("qopenhd_secondary_video_rtp_input_ip",kDefault_udp_rtp_input_ip_address).toString();
...
static GenericVideoSettings read_generic_from_settings(){
    ...
    _videoStreamConfig.dev_test_video_mode=OpenHDVideoHelper::videoTestModeFromInt(settings.value("dev_test_video_mode", 0).toInt());
    ...
    // QML text input sucks, so we read a file. Not ideal, but for testing only anyways
    {
        ...
        if(_videoStreamConfig.dev_enable_custom_pipeline){
            ...
            if(file.is_open()){
                ...
                std::cout<<"dev_enable_custom_pipeline but no file"<<std::endl;
            }
        }
    }
...
static std::string get_default_openhd_test_file(const VideoCodec video_codec){
    ...
    if(video_codec==OpenHDVideoHelper::VideoCodecH264){
        in_filename<<"rpi_1080.h264";
    }else if(video_codec==OpenHDVideoHelper::VideoCodecH265){
...
static std::string create_udp_rtp_sdp_file(const VideoStreamConfigXX& video_stream_config){
    ...
    //ss<<"width=1280\n";
    //ss<<"height=720\n";
    if(video_stream_config.video_codec==OpenHDVideoHelper::VideoCodec::VideoCodecMJPEG){
        ...
        if(video_stream_config.video_codec==OpenHDVideoHelper::VideoCodec::VideoCodecH264){
            ss<<"a=rtpmap:96 H264/90000\n";
        }else{
            assert(video_stream_config.video_codec==OpenHDVideoHelper::VideoCodec::VideoCodecH265);
