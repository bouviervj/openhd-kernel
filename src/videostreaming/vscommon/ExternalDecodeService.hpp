...
static void write_service_rotation_file(int rotation){
    std::cout<<"Writing "<<rotation<<"° to video service file"<<std::endl;
...
static std::string get_service_name_for_codec(const OpenHDVideoHelper::VideoCodec& codec){
    if(codec==OpenHDVideoHelper::VideoCodecH264)return GENERIC_H264_DECODE_SERVICE;
    if(codec==OpenHDVideoHelper::VideoCodecH264)return GENERIC_H265_DECODE_SERVICE;
...
static void write_service_environment_file(const OpenHDVideoHelper::VideoStreamConfigXX& config){
    std::stringstream ss;
    ss<<"ROTATION="<<0<<"\n";
...
void decode_via_external_decode_service(const OpenHDVideoHelper::VideoStreamConfig& settings,std::atomic<bool>& request_restart){
    std::cout<<"dirty_generic_decode_via_external_decode_service begin"<<std::endl;
    ...
    DecodingStatistcs::instance().reset_all_to_default();
    {
        ...
        type<<"External "<<OpenHDVideoHelper::video_codec_to_string(settings.primary_stream_config.video_codec);
    ...
    if(!qopenhd_is_background_transparent()){
        ...
        std::cerr<<"Background not transparent !"<<std::endl;
        std::cerr<<"Make transparent (Screen Settings) and restart !"<<std::endl;
    }

    // Dirty way we communicate with the service / executable
    const auto rotation=OpenHDVideoHelper::get_display_rotation();
    ...
    std::cout<<"dirty_generic_decode_via_external_decode_service end"<<std::endl;
}
