...
void TextureRenderer::initGL(void *window)
{
    if (!initialized) {
        ...
        std::cout<<gl_video_renderer->debug_info().c_str()<<std::endl;
...
void TextureRenderer::paint(void *window,int rotation_degree)
{
   ...
   if(video_tex_width >0 && video_tex_height>0){
       const auto viewport=helper::ratio::calculate_viewport(m_viewportSize.width, m_viewportSize.height,video_tex_width,video_tex_height,OpenHDVideoHelper::get_primary_video_scale_to_fit());
