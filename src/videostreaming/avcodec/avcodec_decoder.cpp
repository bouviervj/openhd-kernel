...
#include "../vscommon/ExternalDecodeService.hpp"
...
static enum AVPixelFormat get_hw_format(AVCodecContext *ctx,const enum AVPixelFormat *pix_fmts){
    ...
    std::cout<<"Supported (HW) pixel formats: "<<supported_formats.str().c_str()<<std::endl;
...
static enum AVPixelFormat get_sw_format(AVCodecContext *ctx,const enum AVPixelFormat *pix_fmts){
    const enum AVPixelFormat *p;
    std::cout<<"All (SW) pixel formats:"<<all_formats_to_string(pix_fmts).c_str()<<std::endl;
    for (p = pix_fmts; *p != -1; p++) {
    ...
    std::cout<<"Weird, we should be able to do SW decoding on all platforms"<<std::endl;
...
{
    std::cout<< "AVCodecDecoder::init()" <<std::endl;
    m_last_video_settings=OpenHDVideoHelper::read_config_from_settings();
    decode_thread = std::make_unique<std::thread>([this]{this->constant_decode();} );
    timer_check_settings_changed=std::make_unique<Timer>();
    // Set the timer
    //assert(0);
    std::function<void()> timer_check = std::bind(&AVCodecDecoder::timer_check_settings_changed_callback, this);
    timer_check_settings_changed->setFunc(timer_check);
    //::connect(timer_check_settings_changed.get(), &QTimer::timeout, this, &AVCodecDecoder::timer_check_settings_changed_callback);
...
void AVCodecDecoder::terminate()
{
    ...
    std::cout<<"Decoding terminated"<<std::endl;
}

void AVCodecDecoder::timer_check_settings_changed_callback()
{
    const auto new_settings=OpenHDVideoHelper::read_config_from_settings();
...
void AVCodecDecoder::constant_decode()
{
    while(!m_should_terminate){
        ...
        const auto settings = OpenHDVideoHelper::read_config_from_settings();
        ...
        if(settings.generic.qopenhd_switch_primary_secondary){
         ...
         if(stream_config.video_codec==OpenHDVideoHelper::VideoCodecMJPEG){
        ...
        if(use_external_decode_service){
            ...
            if(settings.generic.dev_test_video_mode!=OpenHDVideoHelper::VideoTestMode::DISABLED){
        ...
        std::cout<<"Decode stopped,restarting"<<std::endl;
    ...
    while (!gotFrame){
        ...
        if(ret == AVERROR_EOF){
            ...
            if(std::chrono::steady_clock::now()-loopUntilFrameBegin > std::chrono::seconds(2)){
              std::cout<<"Got no frame after X seconds. Break, but decode delay will be reported wrong"<<std::endl;
            ...
            std::cout<<"Got unlikely / weird error:"<<ret<<std::endl;
            break;
        }
...
void AVCodecDecoder::fetch_frame_or_feed_input_packet(){
    ...
    while(keep_fetching_frames_or_input_packets){
        ...
        if(ret == AVERROR_EOF){
            ...
            std::cout<<"received frame"<<ret<<std::endl;
            ...
            std::cout<<"Weird decoder error:"<<ret<<std::endl;
            keep_fetching_frames_or_input_packets=false;
        }
    }
    av_packet_free(&pkt);
...
{
    {
        ...
        DecodingStatistcs::instance().set_primary_stream_frame_format(ss.str());
    ...
    if(last_frame_width==-1 || last_frame_height==-1){
        ...
        if(last_frame_width!=frame->width || last_frame_height!=frame->height){
            ...
            std::cout<<"Need to restart the decoder, width / heght changed"<<std::endl;
            request_restart=true;
        }
    }
...
int AVCodecDecoder::open_and_decode_until_error(const OpenHDVideoHelper::VideoStreamConfig settings)
{
    // this is always for primary video, unless switching is enabled
    auto stream_config=settings.primary_stream_config;
    if(settings.generic.qopenhd_switch_primary_secondary){
    ...
    if(settings.generic.dev_test_video_mode==OpenHDVideoHelper::VideoTestMode::DISABLED){
        in_filename=OpenHDVideoHelper::get_udp_rtp_sdp_filename(stream_config);
        ...
        if(settings.generic.dev_enable_custom_pipeline){
            ...
            if(consti_testing){
                if(stream_config.video_codec==OpenHDVideoHelper::VideoCodecH264){
                ...
                }else if(stream_config.video_codec==OpenHDVideoHelper::VideoCodecH265){
                ...
                in_filename=OpenHDVideoHelper::get_default_openhd_test_file(stream_config.video_codec);
            }
    ...
    if (avformat_open_input(&input_ctx,in_filename.c_str(), NULL, &av_dictionary) != 0) {
        std::cout<< "Cannot open input file ["<<in_filename.c_str()<<"]"<<std::endl;
    ...
    std::cout<<"done avformat_open_input["<<in_filename.c_str()<<"]"<<std::endl;

    if (avformat_find_stream_info(input_ctx, NULL) < 0) {
        std::cout<< "Cannot find input stream information." <<std::endl;
        avformat_close_input(&input_ctx);
        return -1;
    }
    std::cout<<"done avformat_find_stream_info"<<std::endl;
    int ret=0;
...
#endif
    if (ret < 0) {
        std::cout<< "Cannot find a video stream in the input file"<<std::endl;
        avformat_close_input(&input_ctx);
        return -1;
    }
    std::cout<<"done av_find_best_stream:"<<ret<<std::endl;
    const int video_stream = ret;

    if(!(decoder->id==AV_CODEC_ID_H264 || decoder->id==AV_CODEC_ID_H265 || decoder->id==AV_CODEC_ID_MJPEG)){
      std::cout<<"We only do h264,h265 and mjpeg in this project"<<std::endl;
      avformat_close_input(&input_ctx);
      return 0;
    }
    ...
    if (decoder->id == AV_CODEC_ID_H264) {
        ...
        std::cout<<all_hw_configs_for_this_codec(decoder).c_str()<<std::endl;
    ...
    if(stream_config.enable_software_video_decoder ||
            ...
            stream_config.video_codec==OpenHDVideoHelper::VideoCodecMJPEG
            ){
        std::cout<<"User wants SW decode / mjpeg"<<std::endl;
        decoder_ctx->get_format  = get_sw_format;
        selected_decoding_type="SW";
    }else{
        std::cout<<"Try HW decode"<<std::endl;
        decoder_ctx->get_format  = get_hw_format;
        if (hw_decoder_init(decoder_ctx, kAvhwDeviceType) < 0){
          std::cout<<"HW decoder init failed,fallback to SW decode"<<std::endl;
   ...
   if ((ret = avcodec_open2(decoder_ctx, decoder, nullptr)) < 0) {
        std::cout<<"Failed to open codec for stream "<<std::endl;//<< video_stream;
        avformat_close_input(&input_ctx);
        return -1;
    }
    AVPacket packet;
    ...
    while (ret >= 0) {
        ...
        if ((ret = av_read_frame(input_ctx, &packet)) < 0){
            std::cout<<"av_read_frame returned:"<<ret<<" "<<av_error_as_string(ret).c_str()<<std::endl;
        ...
        if(false){
             std::cout<<"Got "<<debug_av_packet(&packet).c_str()<<std::endl;
        }else{
    ...
    std::cout<<"Broke out of the queue_data_dequeue_frame loop"<<std::endl;
    ...
    std::cout<<"avcodec_free_context done"<<std::endl;
    avformat_close_input(&input_ctx);
    std::cout<<"avformat_close_input_done"<<std::endl;
    return 0;
}


// https://ffmpeg.org/doxygen/3.3/decode_video_8c-example.html
void AVCodecDecoder::open_and_decode_until_error_custom_rtp(const OpenHDVideoHelper::VideoStreamConfig settings)
{
    // this is always for primary video, unless switching is enabled
    auto stream_config=settings.primary_stream_config;
    if(settings.generic.qopenhd_switch_primary_secondary){
    ...
    av_log_set_level(AV_LOG_TRACE);
     assert(stream_config.video_codec==OpenHDVideoHelper::VideoCodecH264 || stream_config.video_codec==OpenHDVideoHelper::VideoCodecH265);
     if(stream_config.video_codec==OpenHDVideoHelper::VideoCodecH264){
     ...
     if (!decoder) {
         std::cout<< "AVCodecDecoder::open_and_decode_until_error_custom_rtp: Codec not found"<<std::endl;
         return;
     }
     // ----------------------
     ...
     if (!decoder_ctx) {
         std::cout<< "AVCodecDecoder::open_and_decode_until_error_custom_rtp: Could not allocate video codec context" << std::endl;
         return;
     }
     // ----------------------------------
    // From moonlight-qt. However, on PI, this doesn't seem to make any difference, at least for H265 decode.
    ...
    // ---------------------------------------
     ...
     std::cout<<"AVCodecDecoder::open_and_decode_until_error_custom_rtp()-begin loop" <<std::endl;
#ifdef HACK_RAW
     ...
     while(true){
         ...
         if(m_rtp_receiver->config_has_changed_during_decode){
             std::cout<<"Break/Restart,config has changed during decode"<<std::endl;
             goto finish;
         }
         ...
         if(!has_keyframe_data){
              ...
              if(keyframe_buf==nullptr){
                  ...
                  std::cout<<"No buff after 100ms."<<std::endl;
                  continue;
              }
              std::cout<<"Got decode data (before keyframe)"<<std::endl;
...
finish:
     std::cout<<"AVCodecDecoder::open_and_decode_until_error_custom_rtp()-end loop"<<std::endl;
...
void AVCodecDecoder::timestamp_debug_valid(int64_t ts)
{
    ...
    if(valid){
        std::cout<<"Is a valid timestamp"<<std::endl;
    }else{
        std::cout<<"Is not a valid timestamp"<<std::endl;
    }
}

void AVCodecDecoder::dirty_generic_decode_via_external_decode_service(const OpenHDVideoHelper::VideoStreamConfig& settings)
{
