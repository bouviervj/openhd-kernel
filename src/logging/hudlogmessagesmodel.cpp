...
HUDLogMessagesModel::HUDLogMessagesModel()
{
    ...
    std::function<void()> timer_check = std::bind(&HUDLogMessagesModel::handle_cleanup, this);
    m_cleanup_timer.setFunc(timer_check);
    m_cleanup_timer.start(1000);

    //m_cleanup_timer = new Timer();
...
void HUDLogMessagesModel::add_message(int severity, const std::string& message)
{
    /*emit*/ signalAddLogMessage(severity,message);
}

void HUDLogMessagesModel::add_message_info(const std::string& message)
{
    add_message(6,message);
}

void HUDLogMessagesModel::add_message_warning(const std::string& message)
{
...
void HUDLogMessagesModel::signalAddLogMessage(int severity, const std::string& message){
    do_not_call_me_addLogMessage(severity,message);
}

void HUDLogMessagesModel::do_not_call_me_addLogMessage(int severity, const std::string& message){
