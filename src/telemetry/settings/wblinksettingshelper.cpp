...
static void tmp_log_result(bool enable,const std::string message){
    if(enable){
        ...
        std::cout << "Not logged to HUD: " << message.c_str() << std::endl;
    }
}

WBLinkSettingsHelper::WBLinkSettingsHelper()
{
}
...
void WBLinkSettingsHelper::validate_and_set_gnd_channel_mhz(int channel_mhz)
{
    if(channel_mhz<=1000 || channel_mhz>=8000){
        std::cout << "Invalid channel " << channel_mhz << " Mhz" << std::endl;
        return;
    }
    if(m_curr_channel_mhz!=channel_mhz){
        std::cout << "Changing channel from " << m_curr_channel_mhz << " to " << channel_mhz << std::endl;
...
void WBLinkSettingsHelper::validate_and_set_gnd_channel_width_mhz(int channel_width_mhz)
{
    if(!(channel_width_mhz==10 || channel_width_mhz==20 || channel_width_mhz==40 || channel_width_mhz==80)){
        std::cout << "Invalid channel width " << channel_width_mhz << " Mhz" << std::endl;
        return;
    }
    if(m_curr_channel_width_mhz!=channel_width_mhz){
        std::cout << "Changing channel width from " << m_curr_channel_width_mhz << " to " << channel_width_mhz << std::endl;
...
void WBLinkSettingsHelper::process_message_openhd_wifibroadcast_supported_channels(const mavlink_openhd_wifbroadcast_supported_channels_t &msg)
{
    ...
    if(channels.empty()){
        std::cout << "No valid channels from ground station - should never happen" << std::endl;
        return;
    }
    if(FrequencyHelper::instance().set_hw_supported_frequencies_threadsafe(channels)){
        std::cout << "Supported channels changed" << std::endl;
        signal_ui_rebuild_model_when_possible();
    }
}
...
void WBLinkSettingsHelper::process_message_openhd_wifibroadcast_analyze_channels_progress(const mavlink_openhd_wifbroadcast_analyze_channels_progress_t &msg)
{
    ...
    if(analyzed_channels.size()==0){
        std::cout << "Perhaps malformed message analyze channels" << std::endl;
        return;
    }
...
int WBLinkSettingsHelper::change_param_air_and_ground_blocking(std::string param_id,int value)
{
    std::cout << "SynchronizedSettings::change_param_air_and_ground: " << param_id << ":" << value << std::endl;
    ...
    const auto command_air=XParam::create_cmd_set_int(OHD_SYS_ID_AIR,OHD_COMP_ID_LINK_PARAM,param_id,value);
    const bool air_success=XParam::instance().try_set_param_blocking(command_air,std::chrono::milliseconds(100),20);
    if(!air_success){
        std::stringstream ss;
        ss<<"Cannot change "<<param_id<<" to "<<value<<" air not reached";
    std::cout << ss.str().c_str() << std::endl;
    ...
    std::stringstream ss;
    ss<<"Successfully changed "<<param_id<<" to "<<value<<" ,might take up to 3 seconds until applied";
    std::cout << ss.str().c_str() << std::endl;
    return 0;
}

bool WBLinkSettingsHelper::change_param_ground_only_blocking(std::string param_id, int value)
{
    const auto command_gnd=XParam::create_cmd_set_int(OHD_SYS_ID_GROUND,OHD_COMP_ID_LINK_PARAM,param_id,value);
    const bool ground_success=XParam::instance().try_set_param_blocking(command_gnd,std::chrono::milliseconds(200),5);
    if(ground_success){
    std::cout << "change_param_ground_only success " << param_id << ":" << value << std::endl;
        return true;
    }
    std::cout << "change_param_ground_only failure " << param_id << ":" << value << std::endl;
    return false;
}
...
void WBLinkSettingsHelper::set_param_video_resolution_framerate_async(bool primary,std::string res_str)
{
    const std::string value=res_str;
...
{
    std::cout << "set_param_tx_power " << (is_tx_power_index ? "IDX" : "MW") << " " << (is_for_armed_state ? "ARMED" : "DISARMED") << " " << value << std::endl;
    auto& system=ground ? AOHDSystem::instanceGround() : AOHDSystem::instanceAir();
    if(!system.m_is_alive){
        return false;
    }
    std::string param_id="";
...
{
    std::cout << "set_param_stbc_ldpc_enable_air_ground" << std::endl;
...
void WBLinkSettingsHelper::change_param_air_async(const int comp_id,const std::string param_id,std::variant<int32_t,std::string> param_value,const std::string tag)
{
    ...
    if(!AOHDSystem::instanceAir().m_is_alive){
        HUDLogMessagesModel::instance().add_message_warning("Air not alive - cannot change "+std::string(tag.c_str()));
        return;
    }
    ...
    if(!enqueue_success){
        HUDLogMessagesModel::instance().add_message_warning("Busy - cannot change "+std::string(tag.c_str())+", try again later");
        return;
    }
}
...
void WBLinkSettingsHelper::signal_ui_rebuild_model_when_possible()
{
    ...
    if(m_curr_channel_mhz>0 && m_curr_channel_width_mhz>0 && valid_ground_channel_data){
         std::cout << "Signal UI Ready & should rebuild " << m_curr_channel_mhz << ":" << m_curr_channel_width_mhz << " Mhz valid channels: " << valid_ground_channel_data << std::endl;
         set_ui_rebuild_models(m_ui_rebuild_models+1);
     }else{
         std::cout << "Signal UI Not ready yet " << m_curr_channel_mhz << ":" << m_curr_channel_width_mhz << " Mhz valid channels: " << valid_ground_channel_data << std::endl;
     }
}
