...
bool MavlinkSettingsModel::system_is_alive()
{
    if(m_sys_id==OHD_SYS_ID_AIR){
        return AOHDSystem::instanceAir().m_is_alive;
    }
    return AOHDSystem::instanceGround().m_is_alive;
}
...
MavlinkSettingsModel::MavlinkSettingsModel(uint8_t sys_id,uint8_t comp_id)
    : m_sys_id(sys_id),m_comp_id(comp_id)
{
...
{
    std::cout << "MavlinkSettingsModel::try_fetch_all_parameters()" << std::endl;
    if(!m_is_ready){
        // not discovered yet
        if(log_result){
...
MavlinkSettingsModel::SetParamResult MavlinkSettingsModel::try_set_param_int_impl(const std::string param_id, int value)
{
    auto command=XParam::create_cmd_set_int(m_sys_id,m_comp_id,param_id,value);
    m_is_currently_busy=true;
...
MavlinkSettingsModel::SetParamResult MavlinkSettingsModel::try_set_param_string_impl(const std::string param_id,std::string value)
{
    auto command=XParam::create_cmd_set_string(m_sys_id,m_comp_id,param_id,value);
    m_is_currently_busy=true;
    const auto result=XParam::instance().try_set_param_blocking(command);
    m_is_currently_busy=false;
    return result ? SetParamResult::SUCCESS : SetParamResult::NO_CONNECTION;
...
void MavlinkSettingsModel::try_set_param_int_async(const std::string param_id, int value,bool log_result)
{
    if(m_is_currently_busy){
    ...
    std::cout << "Result: " << m_comp_id << ":" << result.is_accepted() << std::endl;
       m_is_currently_busy=false;
       set_ui_is_busy(false);
...
void MavlinkSettingsModel::try_set_param_string_async(const std::string param_id,std::string value,bool log_result)
{
    if(m_is_currently_busy){
    std::cout << "BUSY" << std::endl;
...
std::string MavlinkSettingsModel::try_update_parameter_int(const std::string param_id,int value)
{
    std::cout << "try_update_parameter_int: " << param_id << "," << value << std::endl;
    ...
    std::cout << "Failure code: " << set_param_result_as_string(result).c_str() << std::endl;
    if(result==SetParamResult::VALUE_UNSUPPORTED){
        std::stringstream ss;
        ss<<"Your HW does not support "<<param_id<<"="<<value;
        return ss.str().c_str();
    }
...
std::string MavlinkSettingsModel::try_update_parameter_string(const std::string param_id,std::string value)
{
    std::cout << "try_update_parameter_string: " << param_id << "," << value << std::endl;
...
QVariant MavlinkSettingsModel::data(const QModelIndex &index, int role) const
{
    ...
    if ( role == UniqueIdRole ){
        ...
        return std::string(std::get<std::string>(data.value).c_str());
        ...
        std::string ret=DocumentedParam::get_short_description(data.unique_id.toStdString()).c_str();
        return ret;
...
void MavlinkSettingsModel::addData(MavlinkSettingsModel::SettingData data)
{
    ...
    if(is_param_whitelisted(data.unique_id)){
...
std::string MavlinkSettingsModel::int_enum_get_readable(std::string param_id, int value)const
{
    auto as_enum=DocumentedParam::int_param_to_enum_string_if_known(param_id,value);
    if(as_enum.has_value()){
        return std::string(as_enum.value().c_str());
    }
    std::stringstream ss;
    ss<<"{"<<value<<"}";
...
std::string MavlinkSettingsModel::string_enum_get_readable(std::string param_id,std::string value) const
{
    auto as_enum=DocumentedParam::string_param_to_enum_string_if_known(param_id,value);
    if(as_enum.has_value()){
        return std::string(as_enum.value().c_str());
    }
    std::stringstream ss;
...
bool MavlinkSettingsModel::int_param_has_min_max(std::string param_id) const
{
    const auto improved_opt=DocumentedParam::get_improved_for_int(param_id);
...
int MavlinkSettingsModel::int_param_get_min_value(std::string param_id)const
{
    const auto improved_opt=DocumentedParam::get_improved_for_int(param_id);
    if(improved_opt.has_value()){
...
int MavlinkSettingsModel::int_param_get_max_value(std::string param_id)const
{
    const auto improved_opt=DocumentedParam::get_improved_for_int(param_id);
    if(improved_opt.has_value()){
        if(improved_opt->has_enum_mapping()){
...
bool MavlinkSettingsModel::int_param_has_enum_keys_values(std::string param_id)const
{
    const auto improved_opt=DocumentedParam::get_improved_for_int(param_id);
    if(improved_opt.has_value()){
        if(improved_opt->has_enum_mapping()){
...
std::vector<std::string> MavlinkSettingsModel::int_param_get_enum_keys(std::string param_id) const
{
    const auto improved_opt=DocumentedParam::get_improved_for_int(param_id);
    if(improved_opt.has_value()){
    ...
    std::cout << "Error no enum mapping for this int param" << std::endl;
    }else{
    std::cout << "Error not an int param" << std::endl;
    }
    std::vector<std::string> ret{"Err(0)"};
    return ret;
}

std::vector<int> MavlinkSettingsModel::int_param_get_enum_values(std::string param_id) const
{
    const auto improved_opt=DocumentedParam::get_improved_for_int(param_id);
    if(improved_opt.has_value()){
        const auto improved=improved_opt.value();
...
MavlinkSettingsModel::ParamIntEnum MavlinkSettingsModel::int_param_get_enum(std::string param_id) const
{
...
bool MavlinkSettingsModel::string_param_has_enum(std::string param_id) const
{
    const auto improved_opt=DocumentedParam::get_improved_for_string(param_id);
    if(improved_opt.has_value()){
        return true;
    }
    return false;
...
std::vector<std::string> MavlinkSettingsModel::string_param_get_enum_keys(std::string param_id) const
{
    const auto improved_opt=DocumentedParam::get_improved_for_string(param_id);
    if(improved_opt.has_value()){
    ...
    std::cout << "Error no enum mapping for this int param" << std::endl;
    std::vector<std::string> ret{"ERROR_KEYS"};
    return ret;
}

std::vector<std::string> MavlinkSettingsModel::string_param_get_enum_values(std::string param_id) const
{
    const auto improved_opt=DocumentedParam::get_improved_for_string(param_id);
    if(improved_opt.has_value()){
    ...
    std::cout << "Error no enum mapping for this int param" << std::endl;
    std::vector<std::string> ret{"ERROR_VALUES"};
    return ret;
}

MavlinkSettingsModel::ParamStringEnum MavlinkSettingsModel::string_param_get_enum(std::string param_id) const
{
...
std::string MavlinkSettingsModel::get_warning_before_safe(const std::string param_id)
{
...
bool MavlinkSettingsModel::get_param_requires_manual_reboot(std::string param_id)
{
    return DocumentedParam::requires_reboot(param_id);
}

bool MavlinkSettingsModel::param_int_exists(std::string param_id)
{
    ...
    std::cout << "int Param: " << param_id << " does not exist" << std::endl;
    return false;
}

bool MavlinkSettingsModel::param_string_exists(std::string param_id)
{
    for(const auto& tmp:m_data){
    ...
    std::cout << "string Param: " << param_id << " does not exist" << std::endl;
    return false;
}

int MavlinkSettingsModel::get_cached_int(std::string param_id)
{
    for(const auto& tmp:m_data){
    ...
    std::cout << param_id << " NOT FOUND" << std::endl;
    return -1;
}

std::string MavlinkSettingsModel::get_cached_string(std::string param_id)
{
    for(const auto& tmp:m_data){
...
{
    //std::cout << "Replacing full param set, previous size: " << rowCount() << " new size: " << qt_param_set.param_set.size() << std::endl;
    // first, remove anything the QT model has cached
    //while(rowCount()>0){
    //    removeData(rowCount()-1);
    //}
    std::cout << "Done removing old params" << std::endl;
    for(int i=0;i<qt_param_set.param_set.size();i++){
        const auto param=qt_param_set.param_set[i];
        const std::string param_id=param.param_id;
        std::variant<int32_t,std::string> param_value;
        if(param.type==0){
            int32_t value=std::get<int>(param.param_value);
            param_value=value;
        }else{
            std::string value=std::get<std::string>(param.param_value);
            param_value=value;
        }
...
void MavlinkSettingsModel::finalize_update_param(std::string param_id,std::variant<int32_t,std::string> value, bool success,bool log_result)
{
    set_last_updated_param_id(param_id);
    set_last_updated_param_success(success);
    if(log_result){
         std::stringstream ss;
         ss<<"Update "<<param_id<<" to ";
    ...
    std::cout << "Update count: " << m_update_count << std::endl;
}
