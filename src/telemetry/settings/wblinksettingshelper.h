...
class WBLinkSettingsHelper
{
    
public:
    explicit WBLinkSettingsHelper();
    ...
    int m_curr_air_channel_width_mhz = -1;
    void set_curr_air_channel_width_mhz(int value) { m_curr_air_channel_width_mhz = value; }
    // Dirty, incremented to signal to the UI that it should rebuild the model(s)
    int m_ui_rebuild_models = 0;
    void set_ui_rebuild_models(int value) { m_ui_rebuild_models = value; }
    // Scanning
    std::string m_scanning_text_for_ui = "";
    void set_scanning_text_for_ui(std::string value) { m_scanning_text_for_ui = value; }
    int m_scan_progress_perc = 0;
    void set_scan_progress_perc(int value) { m_scan_progress_perc = value; }
    // Analyzing
    int m_analyze_progress_perc = 0;
    void set_analyze_progress_perc(int value) { m_analyze_progress_perc = value; }
    int m_current_analyze_frequency = -1;
    void set_current_analyze_frequency(int value) { m_current_analyze_frequency = value; }
public:
...
public:
     bool start_analyze_channels(int freq_bands);
    // freq_bands:
    // 0: 2.4G and 5.8G
    // 1: 2.4G only
    // 2: 5.8G only
    // similar for channel widths
     bool start_scan_channels(int freq_bands,int channel_widths);
private:
    ...
    int change_param_air_and_ground_blocking(std::string param_id,int value);
    bool change_param_ground_only_blocking(std::string param_id,int value);
...
public:
     int change_param_air_and_ground_frequency(int value){
    ...
    //
     void change_param_air_channel_width_async(int value,bool log_result);

     bool change_param_ground_only_frequency(int value){
    ...
    // and use the broadcasted value(s) to update the UI
     void set_param_keyframe_interval_async(int keyframe_interval);
     void set_param_fec_percentage_async(int percent);
     void set_param_video_resolution_framerate_async(bool primary,std::string res_str);
     void set_param_air_only_mcs_async(int value);
    // Extra
     bool set_param_tx_power(bool ground,bool is_tx_power_index,bool is_for_armed_state,int value);
     bool set_param_stbc_ldpc_enable_air_ground();
private:
