...
{
    if(!FCMavlinkSystem::instance().m_is_alive){
...
static int flight_mode_string_to_int(const std::string& flight_mode,const int mav_type){
    ...
    if(fm_map.find(flight_mode)!=fm_map.end()){
        ...
        if(qopenhd::flight_mode_is_copter((MAV_TYPE)mav_type)){
            ...
            std::cout << "FM mapped not to this mav type: " << (int)mav_type << std::endl;
        }
    }else{
        std::cout << "FM unmapped: " << flight_mode.c_str() << std::endl;
    }
    return -1;
}

void FCAction::flight_mode_cmd_async_string(std::string flight_mode)
{
    auto& fcMavlinkSystem=FCMavlinkSystem::instance();
    if(!fcMavlinkSystem.m_is_alive){
    ...
    const int flight_mode_type=flight_mode_string_to_int(flight_mode,m_ardupilot_mav_type);
...
bool FCAction::has_mapping(std::string flight_mode)
{
    int mapping=flight_mode_string_to_int(flight_mode,m_ardupilot_mav_type);
