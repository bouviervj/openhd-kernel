...
public:
    ...
    void ping_all_systems();
    // re-apply all FC telemetry rate(s)
    void re_apply_rates();
    // Change the conenction mode
    void change_telemetry_connection_mode(int mode);
    void change_manual_tcp_ip(std::string ip);
public:
    // A couple of stats exposed as QT properties
    int telemetry_pps_in = -1;
    void set_telemetry_pps_in(int value){
        telemetry_pps_in = value;
    }

    int telemetry_bps_in = -1;
    void set_telemetry_bps_in(int value){
        telemetry_bps_in = value;
    }
    bool udp_localhost_mode_enabled = true;
    void set_udp_localhost_mode_enabled(bool value){
        udp_localhost_mode_enabled = value;
    }
    //
    std::string telemetry_connection_status = "N/A";
    void set_telemetry_connection_status(const std::string& value){
        telemetry_connection_status = value;
    }
