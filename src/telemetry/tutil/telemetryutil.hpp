...
static std::string battery_gauge_glyph_from_percentage(int percent){
    percent = (percent / 5) * 5;
...
static std::string px4_mode_from_custom_mode(int custom_mode){
...
static XLogMessage statustext_convert(const mavlink_statustext_t& parsedMsg){
    std::vector<char> tmp_msg(parsedMsg.text,parsedMsg.text+50);
    // ensure null termination
    // some firmwares (like px4) don't null terminate the string if it is exactly
    tmp_msg.push_back(0);
...
static std::string mavlink_gps_fix_type_to_string(const uint8_t fix_type){
    switch(fix_type){
...
static std::string mavlink_mission_type_to_string(const uint8_t mission_type){
    switch(mission_type){
...
static std::string mavlink_frame_to_string(const uint8_t frame){
    switch(frame){
...
static std::string bitrate_bps_to_std_string(int64_t bitrate_bits_per_second){
    return std::string{StringHelper::bitrate_to_string(bitrate_bits_per_second).c_str()};
}
static std::string bitrate_kbits_to_std_string(int64_t bitrate_kbits_per_second){
    return std::string{StringHelper::bitrate_to_string(bitrate_kbits_per_second*1000).c_str()};
}

static std::string pps_to_string(int64_t packets_per_second){
    return std::string((std::to_string(packets_per_second)+" pps").c_str());
}
static std::string us_min_max_avg_to_string(int32_t min_us,int32_t max_us,int32_t avg_us){
    std::stringstream ss;
...
static std::string min_max_avg_to_string(int32_t min,int32_t max,int32_t avg){
    std::stringstream ss;
    ss<<"min:"<<min<<", ";
    ss<<"max:"<<max<<", ";
    ss<<"avg:"<<avg;
