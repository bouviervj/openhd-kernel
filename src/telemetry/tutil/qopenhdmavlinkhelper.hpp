...
static std::string safe_string(const char* text,int text_size){
     std::vector<char> param_id(text, text+ text_size);
     /*
      * If there's no null in the text array, the mavlink docs say it has to be exactly 50 characters,
      * so we add a null to the end and then continue. This guarantees that std::string below will always find
      * a null terminator.
      *
      */

    auto it = std::find(param_id.begin(), param_id.end(), '\0');

    if (it == param_id.end()) {
        param_id.push_back('\0');
    }
    std::string s(param_id.data());
    return s;
}

static std::string debug_mavlink_message(const mavlink_message_t& msg){
    std::stringstream ss;
