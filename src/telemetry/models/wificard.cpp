...
static std::string tx_power_unit_for_card(const int card_type){
    std::stringstream ss;
    if(card_type==0){
        // OpenHD RTL8812AU
        return "TPI";
...
WiFiCard::WiFiCard(bool is_air,int card_idx)
    : m_is_air_card(is_air),m_card_idx(card_idx)
{
    if(!m_is_air_card){
        m_alive_timer = std::make_unique<Timer>();
        std::function<void()> timer_check = std::bind(&WiFiCard::update_alive, this);
        m_alive_timer->setFunc(timer_check);
        //::connect(m_alive_timer.get(), &QTimer::timeout, this, &WiFiCard::update_alive);
...
int WiFiCard::helper_get_gnd_curr_best_rssi()
{
    ...
    for(int i=0;i<4;i++){
        ...
        if(card.m_alive && card_rssi>best_rssi){
            best_rssi=card_rssi;
        }
    }
    return best_rssi;
