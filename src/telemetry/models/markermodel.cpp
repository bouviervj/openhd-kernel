...
Traffic::Traffic(const std::string &callsign, const int &contact, const double &lat, const double &lon,
...
std::string Traffic::callsign() const{
    return m_callsign;
}
...
MarkerModel::MarkerModel()/*: QAbstractListModel(parent)*/ {
    std::cout << "MarkerModel::MarkerModel()" << std::endl;
}

void MarkerModel::initMarkerModel() {
    std::cout << "MarkerModel::initMarkerModel()" << std::endl;
