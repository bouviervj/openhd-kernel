...
AOHDSystem::AOHDSystem(const bool is_air)
    : m_is_air(is_air)
{
    m_alive_timer = std::make_unique<Timer>();
    std::function<void()> timer_check = std::bind(&AOHDSystem::update_alive, this);
    m_alive_timer->setFunc(timer_check);
    //::connect(m_alive_timer.get(), &QTimer::timeout, this, &AOHDSystem::update_alive);
...
bool AOHDSystem::process_message(const mavlink_message_t &msg)
{
    ...
    std::cout << "AOHDSystem::process_message: wron sys id" << std::endl;
        return false;
    }
    ...
    switch(msg.msgid){
        ...
        case MAVLINK_MSG_ID_OPENHD_CAMERA_STATUS_AIR:{
            ...
            if(msg.compid==OHD_COMP_ID_AIR_CAMERA_PRIMARY){
                ...
                const int value_in_qopenhd=OpenHDVideoHelper::get_qopenhd_n_cameras();
        ...
        case MAVLINK_MSG_ID_HEARTBEAT:{
            ...
            if(parsedMsg.autopilot!=MAV_AUTOPILOT_INVALID){
                std::cout << "Warning OpenHD systems should always set autopilot to none" << std::endl;
            }
            consumed=true;
        }break;
        ...
        case MAVLINK_MSG_ID_OPENHD_WIFBROADCAST_GND_OPERATING_MODE:{
            if(m_is_air) std::cout << "Message mismatch" << std::endl; // only ground reports operating mode
        ...
        default:{
            std::cout << "Unknown openhd message type" << std::endl;
        }break;
        /*case MAVLINK_MSG_ID_OPENHD_LOG_MESSAGE:{
            ...
            const std::string message{parsedMsg.text};
            const long long int timestamp=parsedMsg.timestamp;
...
std::string AOHDSystem::get_rate_for_mcs_bw(int mcs, int bw) const
{
    return "TODO";
}
...
void AOHDSystem::process_x0(const mavlink_openhd_stats_monitor_mode_wifi_card_t &msg){
    ...
    std::cout << "Air only has 1 wifibroadcats card" << std::endl;
        return;
    }
    ...
    if(m_is_air){
        ...
        set_current_rx_rssi(card.m_curr_rx_rssi_dbm);
    }else{
        if(msg.card_index<0 || msg.card_index>=4){
            std::cout << "Gnd invalid card index " << msg.card_index << std::endl;
            return;
        }
...
void AOHDSystem::process_x2(const mavlink_openhd_stats_telemetry_t &msg)
{
    ...
    set_curr_telemetry_rx_bps(Telemetryutil::bitrate_bps_to_std_string(msg.curr_rx_bps));
    set_curr_telemetry_tx_bps(Telemetryutil::bitrate_bps_to_std_string(msg.curr_tx_bps));
...
void AOHDSystem::process_x3(const mavlink_openhd_stats_wb_video_air_t &msg){
    ...
    if(!m_is_air){
    std::cout << "warning got mavlink_openhd_stats_wb_video_air from ground" << std::endl;
        return;
    }
...
{
    if(!m_is_air){
    std::cout << "warning got mavlink_openhd_stats_wb_video_air_fec_performance_t from ground" << std::endl;
        return;
    }
    if(msg.link_index==0 || msg.link_index==1){
        auto& cam=CameraStreamModel::instance(msg.link_index);
...
void AOHDSystem::process_x4(const mavlink_openhd_stats_wb_video_ground_t &msg){
    ...
    if(m_is_air){
         std::cout << "warning got mavlink_openhd_stats_wb_video_ground from air" << std::endl;
         return;
    }
    if(msg.link_index==0 || msg.link_index==1){
        auto& cam=CameraStreamModel::instance(msg.link_index);
...
{
    if(m_is_air){
    std::cout << "warning got mavlink_openhd_stats_wb_video_ground_fec_performance_ from air" << std::endl;
        return;
    }
    if(msg.link_index==0 || msg.link_index==1){
        auto& cam=CameraStreamModel::instance(msg.link_index);
        ...
        if(m_wb_gnd_operating_mode==0){
            ...
            if(!MavlinkSettingsModel::is_air_or_cam_param_busy()){
                ...
                if(!MavlinkSettingsModel::instanceGround().has_params_fetched && !MavlinkSettingsModel::instanceGround().is_x_busy()){
                    std::cout << "Init autofetch of groun param set" << std::endl;
            ...
            if(m_curr_rx_last_packet_status_good){
                ...
                if(!MavlinkSettingsModel::is_air_or_cam_param_busy()){
                    ...
                    if(!MavlinkSettingsModel::instanceAir().has_params_fetched){
                        std::cout << "Init autofetch of air param set" << std::endl;
                        ...
                        std::cout << "Init autofetch of air camera1 param set" << std::endl;
                        ...
                        std::cout << "Init autofetch of air camera2 param set" << std::endl;
