...
CameraStreamModel::CameraStreamModel(int m_camera_index)
...
std::string CameraStreamModel::camera_type_to_string(int camera_type)
{
...
std::string CameraStreamModel::camera_status_to_string(int camera_status)
{
...
std::string CameraStreamModel::camera_codec_to_string(int camera_codec)
{
...
std::string CameraStreamModel::camera_recording_mode_to_string(int recording_mode)
{
...
bool CameraStreamModel::is_valid_resolution_fps_string(std::string input)
{
    auto parsed=parse_video_format(input);
    if(parsed.has_value())return true;
    return false;
}

std::string CameraStreamModel::get_default_resolution()
{
...
std::vector<std::string> CameraStreamModel::get_supported_resolutions()
{
...
std::string CameraStreamModel::make_resolution_fps_verbose(std::string input)
{
    auto parsed=parse_video_format(input);
    if(parsed.has_value()){
        return get_verbose_string_of_resolution(parsed.value()).c_str();
    }
    std::cout << "WEIRD RESOLUTION " << input << std::endl;
    return input;
}
...
void CameraStreamModel::update_mavlink_openhd_stats_wb_video_air(const mavlink_openhd_stats_wb_video_air_t &msg)
{
    ...
    set_curr_video_measured_encoder_bitrate(Telemetryutil::bitrate_bps_to_std_string(msg.curr_measured_encoder_bitrate));
    set_curr_video_injected_bitrate(Telemetryutil::bitrate_bps_to_std_string(msg.curr_injected_bitrate));
...
void CameraStreamModel::update_mavlink_openhd_camera_status_air(const mavlink_openhd_camera_status_air_t &msg)
{
    ...
    if(new_res_fps.is_any_invalid()){
        std::cout << "Invalid data from air unit: " << resolution_framerate_to_string(new_res_fps).c_str() << std::endl;
    }else{
        if(m_curr_res_framerate.is_any_invalid()){
           ...
           if(m_curr_res_framerate!=new_res_fps){
               m_curr_res_framerate=new_res_fps;
               std::cout << "Res/Framerate changed: " << resolution_framerate_to_string(m_curr_res_framerate).c_str() << std::endl;
           }
        }
    }
    // Feature - automatically set the right codec in qopenhd
    const bool secondary=m_camera_index==1;
    const int codec_in_qopenhd=OpenHDVideoHelper::get_qopenhd_camera_video_codec(secondary);
    ...
    if(codec_in_openhd==0 || codec_in_openhd==1 || codec_in_openhd==2){
        if(codec_in_qopenhd!=codec_in_openhd){
           OpenHDVideoHelper::set_qopenhd_camera_video_codec(secondary,codec_in_openhd);
    ...
    std::cout << "Invalid video codec: " << codec_in_openhd << std::endl;
    }
...
{
    set_curr_received_bitrate_with_fec(Telemetryutil::bitrate_bps_to_std_string(msg.curr_incoming_bitrate));
...
void CameraStreamModel::set_curr_recommended_bitrate_from_message(const int64_t curr_recommended_bitrate_kbits)
{
    if(m_curr_recomended_video_bitrate_kbits!= 0 && curr_recommended_bitrate_kbits!=0 && m_curr_recomended_video_bitrate_kbits != curr_recommended_bitrate_kbits){
        std::string message=m_camera_index==0 ? "Cam1 encoder:" : "Cam2 encoder:";
        message+=Telemetryutil::bitrate_kbits_to_std_string(curr_recommended_bitrate_kbits);
    ...
    if(curr_recommended_bitrate_kbits>0){
        set_curr_recomended_video_bitrate_string(Telemetryutil::bitrate_kbits_to_std_string(curr_recommended_bitrate_kbits));
    }else{
...
std::vector<QCameraNameAndType> CameraStreamModel::get_camera_choices(int platform_type)
{
    std::vector<QCameraNameAndType> list{
...
std::vector<std::string> CameraStreamModel::get_manufacturer_choices(int platform_type)
{
...
std::vector<int> CameraStreamModel::get_manufacturer_cameras_type(int platform_type, int index)
{
    auto all_choices=get_camera_choices_for_platform(platform_type,m_camera_index!=0);
    std::vector<int> ret;
...
std::vector<std::string> CameraStreamModel::get_manufacturer_cameras_names(int platform_type, int index)
{
    auto all_choices=get_camera_choices_for_platform(platform_type,m_camera_index!=0);
    std::vector<std::string> ret;
    if(index>=all_choices.size())index=0;
