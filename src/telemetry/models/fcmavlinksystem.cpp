...
FCMavlinkSystem::FCMavlinkSystem() {
    m_flight_time_timer = std::make_unique<Timer>();
    assert(0);
    //::connect(m_flight_time_timer.get(), &QTimer::timeout, this, &FCMavlinkSystem::updateFlightTimer);
    m_flight_time_timer->start(1000);
    m_alive_timer = std::make_unique<Timer>();
    assert(0);
    //::connect(m_alive_timer.get(), &QTimer::timeout, this, &FCMavlinkSystem::update_alive);
...
bool FCMavlinkSystem::process_message(const mavlink_message_t &msg)
{
    ...
    if(!opt_set_sys_id.has_value()){
        std::cout << "WARNING the system must be set before FC model starts processing data" << std::endl;
        return false;
    }
    const auto set_sys_id=opt_set_sys_id.value();
    if(msg.sysid!=set_sys_id){
        std::cout << "Do not pass messages not coming from the FC to the FC model" << std::endl;
        return false;
    }
    ...
    case MAVLINK_MSG_ID_HEARTBEAT: {
        ...
        if(opt_info.has_value()){
            ...
            std::cout << "Weird heartbeat" << std::endl;
        }
        m_n_heartbeats++;
    ...
    std::cout << "MAVLINK AUTOPILOT board_version= " << ap_board_version << std::endl;
    std::cout << "MAVLINK AUTOPILOT os_version= " << ap_os_version << std::endl;
    std::cout << "MAVLINK AUTOPILOT product_id= " << ap_product_id << std::endl;
    std::cout << "MAVLINK AUTOPILOT uid= " << ap_uid << std::endl;
        break;
    }
    case MAVLINK_MSG_ID_SYS_STATUS: {
        ...
        if(battery_remaining_perc != -1){
            ...
            const std::string fc_battery_gauge_glyph = Telemetryutil::battery_gauge_glyph_from_percentage(battery_remaining_perc);
    ...
    case MAVLINK_MSG_ID_SYSTEM_TIME:{
        ...
        uint64_t time;
        time = sys_time.time_unix_usec/1000;



        set_sys_time_unix_as_str(millisToFormattedString(time));
    ...
    case MAVLINK_MSG_ID_BATTERY_STATUS: {
        ...
        if(!air_battery_use_batt_id_0_only){
           ...
           const std::string fc_battery_gauge_glyph = Telemetryutil::battery_gauge_glyph_from_percentage(battery_status.battery_remaining);
           set_battery_percent_gauge(fc_battery_gauge_glyph);
        }
    ...
    std::cout << "FCMavlinkSystem received unmatched message with ID " << msg.msgid
          << ", sequence: " << msg.seq
          << " from component " << msg.compid
          << " of system " << msg.sysid << std::endl;
        break;
    }
    }
...
bool FCMavlinkSystem::set_system_id(int sys_id)
{
    ...
    if(sys_id<0 || sys_id >= UINT8_MAX){
        std::cout << "Invalid sys id" << std::endl;
        return false;
    }
    m_sys_id=sys_id;
    m_discovered=true;
...
void FCMavlinkSystem::updateFlightTimer() {
    if (m_armed == true) {
        ...
        std::string s;
        if (hours > 0) {
            s = Time(0,0,0,0).addSecs(elapsed).toString("hh:mm:ss");
        } else {
            s = Time(0,0,0,0).addSecs(elapsed).toString("mm:ss");
        }
        set_flight_time(s);
    }
}
...
void FCMavlinkSystem::send_message_arm_change(bool armed){
    std::cout << "FCMavlinkSystem::send_message_arm_change: " << armed << std::endl;
    if (armed){
...
void FCMavlinkSystem::set_flight_mode(std::string flight_mode) {
    if(flight_mode==m_flight_mode)return;

    std::stringstream ss;
    ss<<"Flight mode: "<<flight_mode;

    QOpenHD::instance().textToSpeech_sayMessage(ss.str());
...
void FCMavlinkSystem::updateWind(){
    ...
    if (m_vertical_speed_indicator_mps < 1 && m_vertical_speed_indicator_mps > -1){
        ...
        Settings settings;
        auto max_speed = settings.value("wind_max_quad_speed", "3").toDouble();
...
void FCMavlinkSystem::recalculate_efficiency()
{
    ...
    if(delta_distance_m<0){
        std::cout << "Invalid distance delta,resetting: " << delta_distance_m << std::endl;
        m_efficiency_last_distance_m=m_flight_distance_m;
        return;
    }
    if(delta_charge_mah<0){
        std::cout << "Invalid charge delta,resetting " << delta_charge_mah << std::endl;
