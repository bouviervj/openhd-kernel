...
static int tcp_socket_try_connect(const std::string remote_ip, const int remote_port, const int timeout_seconds) {
    std::cout << "Attempting to connect to: " << remote_ip.c_str() << ":" << remote_port << " with timeout: " << timeout_seconds << "s" << std::endl;
    ...
    if (ioctlsocket(sockfd, FIONBIO, &mode) != NO_ERROR) {
        std::cout << "Cannot set non-blocking mode: " << WSAGetLastError() << std::endl;
        closesocket(sockfd);
        WSACleanup();
        return -1;
    }
    ...
    if (fcntl(sockfd, F_SETFL, O_NONBLOCK) < 0) {
        std::cout << "Cannot set non-blocking mode: " << strerror(errno) << std::endl;
        close(sockfd);
        return -1;
    }
#endif
    ...
    if (inet_pton(AF_INET, remote_ip.c_str(), &remote_addr.sin_addr) <= 0) {
        std::cout << "Invalid address: " << remote_ip.c_str() << std::endl;
#ifdef _WIN32
        closesocket(sockfd);
        WSACleanup();
#else
    ...
    if (select_result != 1) {
        std::cout << "Connection timed out or failed." << std::endl;
#ifdef _WIN32
        closesocket(sockfd);
        WSACleanup();
#else
    ...
    if (so_error != 0) {
        std::cout << "Socket error after select: " << WSAGetLastError() << std::endl;
        closesocket(sockfd);
        WSACleanup();
        return -1;
    }
    ...
    if (so_error != 0) {
        std::cout << "Socket error after select: " << strerror(so_error) << std::endl;
        close(sockfd);
        return -1;
    }
#endif
...
static bool linux_send_message(int sockfd, const std::string& dest_ip, const int dest_port, const uint8_t* data, int data_len) {
    ...
    if (send_len != data_len) {
        std::cout << "Failed to send message. Sent: " << send_len << " Expected: " << data_len << std::endl;
        return false;
    }
    return true;
}
...
void TCPConnection::send_message(const mavlink_message_t& msg) {
    ...
    if (m_socket_fd < 0) {
        std::cout << "Cannot send message: Socket not connected." << std::endl;
        return;
    }
    if (!linux_send_message(m_socket_fd, m_remote_ip, m_remote_port, buffer, buffer_len)) {
        std::cout << "Failed to send MAVLink message." << std::endl;
    }
}
...
void TCPConnection::receive_until_stopped() {
    ...
    while (m_keep_receiving) {
        ...
        if (elapsed_last_message > 3 * 1000) {
            std::cout << "No message for more than 3 seconds, disconnecting." << std::endl;
            return;
        }
...
void TCPConnection::loop_connect_receive() {
    ...
    std::cout << "Connection loop ended." << std::endl;
}
