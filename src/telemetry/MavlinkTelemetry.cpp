...
{
    std::cout << "MavlinkTelemetry::start() begin" << std::endl;
    Settings settings;
    ...
    std::string tcp_manual_ip=settings.value("qopenhd_mavlink_connection_manual_tcp_ip","192.168.178.36").toString();
    threadsafe_set_manual_tcp_ip(tcp_manual_ip);
    auto cb_udp=[this](mavlink_message_t msg){
        //std::cout << "MavlinkTelemetry::start() got UDP msg id " << msg.msgid << std::endl;
    ...
    auto cb_tcp=[this](mavlink_message_t msg){
        //std::cout << "MavlinkTelemetry::start() got TCP msg id " << msg.msgid << std::endl;
        process_mavlink_message(msg);
    };
    ...
    m_tcp_connection_custom=std::make_unique<TCPConnection>(cb_tcp,tcp_manual_ip,PORT_OHD_MAVLINK_TCP_SERVER,3);
    m_heartbeat_thread_run=true;
    m_heartbeat_thread=std::make_unique<std::thread>(&MavlinkTelemetry::send_heartbeat_loop,this);
    std::cout << "MavlinkTelemetry::start() end" << std::endl;
}
...
void MavlinkTelemetry::terminate()
{
    ...
    std::cout << "MavlinkTelemetry::stopped" << std::endl;
}

MavlinkTelemetry::~MavlinkTelemetry()
{
    std::cout << "MavlinkTelemetry::~() begin" << std::endl;
    terminate();
    std::cout << "MavlinkTelemetry::~() end" << std::endl;
}
...
bool MavlinkTelemetry::sendMessage(mavlink_message_t msg){
    ...
    if(msg.sysid!=sys_id){
        ...
        std::cout << "WARN Sending message with sys id: " << msg.sysid << " instead of " << sys_id << std::endl;
    }
    if(msg.compid!=comp_id){
        // probably a programming error, the message was not packed with the right comp id
        std::cout << "WARN Sending message with comp id: " << msg.compid << " instead of " << comp_id << std::endl;
    }
    // Prefer udp
    if(m_udp_connection->threadsafe_is_alive()){
        ...
        if(m_tcp_connection_wifi_hs->threadsafe_is_alive()){
            ...
            std::cout << "No connection currently alive" << std::endl;
        }
        return true;
    }
    return false;
    ...
    if(source_sysid ==OHD_SYS_ID_AIR){
        ...
        if(!m_fc_found){
            ...
            if(source_compid==MAV_COMP_ID_AUTOPILOT1){
                ...
                std::cout << "Found betaflight FC: " << source_sysid << std::endl;
                FCMavlinkSystem::instance().set_system_id(source_sysid);
                m_fc_sys_id=source_sysid;
                m_fc_comp_id=source_compid;
                m_fc_found=true;
                ...
                std::cout << "Found INAV FC: " << source_sysid << std::endl;
                FCMavlinkSystem::instance().set_system_id(source_sysid);
                m_fc_sys_id=source_sysid;
                m_fc_comp_id=source_compid;
                m_fc_found=true;
            ...
            {
                std::cout << "Got weird system: " << source_sysid << std::endl;
            }
        }
        ...
        if(fc_sys_id.has_value()){
            if(msg.sysid==fc_sys_id.value()){
               ...
               FCMissionHandler::instance().opt_send_messages();
                ...
                std::cout << "MavlinkTelemetry received unmatched message " << QOpenHDMavlinkHelper::debug_mavlink_message(msg) << std::endl;
            }
        }else{
            // we don't know the FC sys id yet.
            std::cout << "MavlinkTelemetry received unmatched message (FC not yet known) " << QOpenHDMavlinkHelper::debug_mavlink_message(msg) << std::endl;
        }
    }
}
...
void MavlinkTelemetry::process_broadcast_message_fc(const mavlink_message_t &msg)
{
    ...
    std::cout << "MavlinkTelemetry::process_message_fc unmatched message: " << QOpenHDMavlinkHelper::debug_mavlink_message(msg) << std::endl;
}
...
void MavlinkTelemetry::process_message_timesync(const mavlink_message_t &msg)
{
    ...
    std::cout << "Got timesync response with we: " << m_last_timesync_out_us << " msg tc1: " << timesync.tc1 << " ts1: " << timesync.ts1 << std::endl;
        ...
        if(msg.sysid==OHD_SYS_ID_AIR){
            ...
            std::cout << "Got ping from fc" << std::endl;
            // almost 100% from flight controller
            FCMavlinkSystem::instance().set_last_ping_result_flight_ctrl(delta_readable.c_str());
        }
    }else{
    std::cout << "Got timesync but it doesn't match: we: " << m_last_timesync_out_us << " msg tc1: " << timesync.tc1 << " ts1: " << timesync.ts1 << std::endl;
    }
}
...
void MavlinkTelemetry::change_manual_tcp_ip(std::string ip)
{
    threadsafe_set_manual_tcp_ip(ip);
}

void MavlinkTelemetry::send_heartbeat_loop()
{
    while(m_heartbeat_thread_run){
        std::cout<<"send_heartbeat_loop mavlink connection mode:" << m_connection_mode << " isLocalOnlyMode:" << is_localhost_only_usage() << std::endl;
...
bool MavlinkTelemetry::is_localhost_only_usage()
{
    if(QOpenHD::instance().is_android() || QOpenHD::instance().is_mac() || QOpenHD::instance().is_windows()
       || Settings().value("qopenhd_mavlink_connection_force_non_localhost_usage",false).toBool()){
