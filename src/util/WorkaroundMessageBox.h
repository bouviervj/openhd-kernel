...
class WorkaroundMessageBox{
    
public:
    explicit WorkaroundMessageBox();
    ...
    void set_text_and_show(std::string text,int optional_time_until_autoremove_seconds=-1){
    ...
    static void makePopupMessage(std::string text,int optional_time_until_autoremove_seconds=-1){
        WorkaroundMessageBox::instance().set_text_and_show(text,optional_time_until_autoremove_seconds);
    }
public:
    std::string text = "NONE";
    void set_text(std::string value) { text = value; }
    bool visible = false;
    ...
    int remaining_time_seconds = -1;
    void set_remaining_time_seconds(int value) { remaining_time_seconds = value; }
public:
    void okay_button_clicked();
public:
//signals:
    void signal_set_text_and_show(std::string text,int optional_time_until_autoremove);
private:
    void do_not_call_me_set_text_and_show(std::string text,int optional_time_until_autoremove);
    std::mutex m_remove_after_delay_timer_mutex;
    std::unique_ptr<Timer> m_remove_after_delay_timer=nullptr;
